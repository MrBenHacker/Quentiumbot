...
async def on_command_error(ctx, error):
    ...
    if isinstance(ctx.channel, discord.TextChannel):
        ...
        with open("extra/data.json", "r", encoding="utf-8", errors="ignore") as file:
            data = json.load(file, strict=False)
        try:lang_server = data[str(server_id)]["lang_server"]
        except:lang_server = "fr"
    else:
        lang_server = "fr"
...
@client.command(pass_context=True, no_pm=True, aliases=["votes", "voter", "blasonsmax", "voteresults"])
async def vote(ctx, *, args=None):
    ...
    if not ctx.message.author.bot == True:
        if server_id == 371687157817016331: # France Les Cités server ID
            ...
            try:
                with open("blasons.txt", "r", encoding="utf-8", errors="ignore") as file:
                    lines = file.readlines()
            except:
                open("blasons.txt", "a+", encoding="utf-8", errors="ignore").close()
                lines = ""
            ...
            if any(x == ctx.message.author.id for x in authorised):
                if "blasonsmax" in ctx.message.content:
                    lines[0] = args + "\n"
                    with open("blasons.txt", "w", encoding="utf-8", errors="ignore") as file:
            ...
            with open("blasons.txt", "a", encoding="utf-8", errors="ignore") as file:
    ...
    if not ctx.message.author.bot == True:
        if server_id == 391272643229384705: # Insoumis server ID
            list_roles = ["Payday 2", "Diablo 3", "Gta 5", "The division", "Fortnite", "CS GO", "Farming simulator",
                          "Lol", "Dead by daylight", "Destiny 2", "Quake", "left 4 dead 2", "GRID 2", "Steep", "Sea of Thieves"]
            if not args:
